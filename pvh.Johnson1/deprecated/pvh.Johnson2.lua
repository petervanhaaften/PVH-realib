...
function johnsonBuffer()
   ...
   --test values included here
   p.johnsonseq_new[0] = "99"
   p.johnsonseq_new[1] = "88"
   p.johnsonseq_new[2] = "77"
   p.johnsonseq_new[3] = "66"
   for j=0, 3 do --j defines johnsonseq to iterate
      for k=0,3 do --search each sequence and sub values from johynsonseq_new
	 p.johnsonseq[j] = string.gsub(p.johnsonseq[j], k, p.johnsonseq_new[k])
	 k = k+1
      end
      Msg("seq" .. j .. " = " .. p.johnsonseq[j])
      j = j+1
   end

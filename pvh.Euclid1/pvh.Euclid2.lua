...
-- GUI
--------------------------------------------------------------------------------
local ctx = reaper.ImGui_CreateContext('pvh.eudlid2')
...
function loop()
  ...
  --reaper.ImGui_SetNextWindowSize(ctx, 180, 50, reaper.ImGui_Cond_FirstUseEver())
  local visible, open = reaper.ImGui_Begin(ctx, 'pvh.euclid2', true)
  if visible then
    frame()

...
function makenoteBuffer()
   ...
   if lengthChecker == false then
      for i=0, notes-1 do --loops through all notes in take
         -- git pitchNew
         pitchNew = p.johnsonseq_iterated[i+1] 
         -- add octave offset and pitch offset to pitchNew
         pitchNew = pitchNew + (octaveSize * octaveBase) + pitchBase
...
function loop()
   ...
   --set window dimensions
      reaper.ImGui_SetWindowSize(ctx, 275, 334, nil)
      --
      ...
      --reaper.ImGui_SetCursorPos(ctx, (w / 2) - 100, 238)
      --
